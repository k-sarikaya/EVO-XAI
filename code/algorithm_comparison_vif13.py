#!/usr/bin/env python
"""
Evolutionary Algorithm Comparison Study - VIF=13
...
print("[OK] Algorithm Comparison VIF=13 - Libraries loaded")

VIF_THRESHOLD = 13  # Changed from 8 to 13
...
def multicollinearity_filter(X, feature_names, vif_threshold=13, min_features=5):
...
def evaluate_model(X, y):
    ...
    for fold, (tr_idx, te_idx) in enumerate(cv.split(X, y), 1):
        ...
        # n_jobs=1 avoids multiprocessing/pipe issues in restricted Windows environments.
...
def main():
    print("\n" + "="*70)
    print("EVOLUTIONARY ALGORITHM COMPARISON - VIF=13")
    ...
    for name, df in datasets.items():
        ...
        print(f"  VIF Filter (threshold={VIF_THRESHOLD}): {chi_n} -> {vif_n} features")
    ...
    results_df.to_csv('results/algorithm_comparison_vif13.csv', index=False)
    features_df.to_csv('results/algorithm_features_vif13.csv', index=False)
    print("\n[OK] Saved: results/algorithm_comparison_vif13.csv")
    print("[OK] Saved: results/algorithm_features_vif13.csv")
    ...
    for i, algo in enumerate(['NSGA-II', 'DE', 'Hybrid_PSO_DE']):
        ...
        bars = ax.bar(x + i*width - width, counts, width, label=algo, color=colors[algo])
        for bar in bars:
    ...
    ax.set_title(f'Feature Selection Comparison (VIF={VIF_THRESHOLD})', fontsize=14, fontweight='bold')
    ax.set_xticks(x)
    ...
    plt.tight_layout()
    plt.savefig('results/algorithm_features_vif13.png', dpi=300, bbox_inches='tight')
    plt.close()
    print("    [OK] results/algorithm_features_vif13.png")
    
    # Plot 2: Metrics
    fig, axes = plt.subplots(1, 3, figsize=(15, 5))
    
    for ax_idx, metric in enumerate(['MCC', 'Sensitivity', 'Specificity']):
        ax = axes[ax_idx]
        x = np.arange(len(datasets_list))
        width = 0.25
        ...
        for i, algo in enumerate(['NSGA-II', 'DE', 'Hybrid_PSO_DE']):
            ...
            ax.bar(x + i*width - width, values, width, label=algo, color=colors[algo])
    ...
    plt.suptitle(f'Classification Metrics (VIF={VIF_THRESHOLD})', fontsize=14, fontweight='bold')
    plt.tight_layout()
    plt.savefig('results/algorithm_metrics_vif13.png', dpi=300, bbox_inches='tight')
    plt.close()
    print("    [OK] results/algorithm_metrics_vif13.png")
    ...
    sns.heatmap(pivot_mcc, annot=True, fmt='.3f', cmap='RdYlGn', ax=axes[0], 
                vmin=0.4, vmax=0.85, center=0.6)
    axes[0].set_title(f'MCC Heatmap (VIF={VIF_THRESHOLD})', fontsize=12, fontweight='bold')
    ...
    axes[1].set_title(f'Feature Count (VIF={VIF_THRESHOLD})', fontsize=12, fontweight='bold')
    
    plt.tight_layout()
    plt.savefig('results/algorithm_heatmaps_vif13.png', dpi=300, bbox_inches='tight')
    plt.close()
    print("    [OK] results/algorithm_heatmaps_vif13.png")
    
    # ===== SUMMARY =====
    print("\n" + "="*70)
    print(f"RESULTS SUMMARY - VIF THRESHOLD = {VIF_THRESHOLD}")
    print("="*70)
    
    print("\n[FEATURE SELECTION]")
    print("-"*70)
    print(results_df[['Algorithm', 'Dataset', 'Original', 'After_ChiSq', 'After_VIF', 'Final', 'Reduction_%']].to_string(index=False))
    
    print("\n[CLASSIFICATION METRICS]")
    print("-"*70)
    print(results_df[['Algorithm', 'Dataset', 'MCC', 'Sensitivity', 'Specificity', 'F1', 'AUC']].to_string(index=False))
    ...
    print("\n" + "="*70)
    print("[SUCCESS] ALGORITHM COMPARISON VIF=13 COMPLETE")
    print("="*70)
    print("\nOutput files:")
    print("  - results/algorithm_comparison_vif13.csv")
    print("  - results/algorithm_features_vif13.csv")
    print("  - results/algorithm_features_vif13.png")
    print("  - results/algorithm_metrics_vif13.png")
    print("  - results/algorithm_heatmaps_vif13.png")
