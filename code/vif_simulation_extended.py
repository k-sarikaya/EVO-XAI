#!/usr/bin/env python
"""
Extended VIF Threshold Simulation
Tests VIF thresholds: 5, 6, 7, 13
Combines with existing 8-12 results for complete analysis
"""
...
print("[OK] Extended VIF Simulation - Libraries loaded")
...
def main():
    print("\n" + "="*70)
    print("EXTENDED VIF THRESHOLD SIMULATION")
    print("Testing VIF thresholds: 5, 6, 7, 13")
    ...
    # New VIF thresholds to test
    vif_thresholds = [5, 6, 7, 13]
    ...
    new_feature_df = pd.DataFrame(feature_counts)
    
    # Load existing results and combine
    print("\n[2] Combining with existing VIF 8-12 results...")
    try:
        existing_df = pd.read_csv('results/vif_simulation_results.csv')
        combined_df = pd.concat([existing_df, new_results_df], ignore_index=True)
        combined_df = combined_df.sort_values(['VIF_Threshold', 'Dataset'])
    except:
        combined_df = new_results_df
    
    # Save combined results
    combined_df.to_csv('results/vif_simulation_all_results.csv', index=False)
    new_results_df.to_csv('results/vif_simulation_extended.csv', index=False)
    print("    [OK] results/vif_simulation_all_results.csv")
    
    # ===== CREATE COMBINED PLOTS =====
    print("\n[3] Creating combined plots...")
    
    # All VIF thresholds
    all_vif = sorted(combined_df['VIF_Threshold'].unique())
    datasets_list = list(datasets.keys())
    
    # Plot 1: Feature Selection Comparison (all VIF values)
    fig, axes = plt.subplots(1, 3, figsize=(18, 6))
    
    for i, dataset in enumerate(datasets_list):
        df_subset = combined_df[combined_df['Dataset'] == dataset].sort_values('VIF_Threshold')
        
        x = np.arange(len(all_vif))
        width = 0.2
        ...
        ax.set_xlabel('VIF Threshold', fontsize=11)
        ax.set_ylabel('Number of Features', fontsize=11)
        ax.set_title(f'{dataset.title()} Dataset', fontsize=12, fontweight='bold')
        ax.set_xticks(x)
        ax.set_xticklabels(all_vif)
        ax.legend(loc='upper left', fontsize=8)
        ax.set_ylim(0, 25)
    
    plt.suptitle('Feature Selection Across All VIF Thresholds (5-13)', fontsize=14, fontweight='bold')
    plt.tight_layout()
    plt.savefig('results/vif_all_feature_selection.png', dpi=300, bbox_inches='tight')
    plt.close()
    print("    [OK] results/vif_all_feature_selection.png")
    ...
    colors_ds = {'child': '#3498db', 'adolescent': '#e74c3c', 'adult': '#2ecc71'}
    ...
    for ax_idx, metric in enumerate(metrics_to_plot):
        ...
        for dataset in datasets_list:
            df_subset = combined_df[combined_df['Dataset'] == dataset].sort_values('VIF_Threshold')
            ax.plot(df_subset['VIF_Threshold'], df_subset[metric], 
                   marker=markers[dataset], label=dataset.title(), 
                   linewidth=2, markersize=8, color=colors_ds[dataset])
        
        ax.set_xlabel('VIF Threshold', fontsize=11)
        ax.set_ylabel(metric, fontsize=11)
        ax.set_title(f'{metric} vs VIF Threshold', fontsize=12, fontweight='bold')
        ax.set_xticks(all_vif)
        ax.legend()
        ax.grid(True, alpha=0.3)
        ax.set_ylim(0, 1.0)
    
    plt.suptitle('Classification Metrics Across All VIF Thresholds', fontsize=14, fontweight='bold')
    plt.tight_layout()
    plt.savefig('results/vif_all_metrics.png', dpi=300, bbox_inches='tight')
    plt.close()
    print("    [OK] results/vif_all_metrics.png")
    
    # Plot 3: Combined heatmaps
    fig, axes = plt.subplots(1, 2, figsize=(16, 6))
    
    pivot_mcc = combined_df.pivot(index='Dataset', columns='VIF_Threshold', values='MCC')
    pivot_mcc = pivot_mcc[sorted(pivot_mcc.columns)]
    ...
    pivot_features = combined_df.pivot(index='Dataset', columns='VIF_Threshold', values='Final')
    pivot_features = pivot_features[sorted(pivot_features.columns)]
    ...
    plt.tight_layout()
    plt.savefig('results/vif_all_heatmaps.png', dpi=300, bbox_inches='tight')
    plt.close()
    print("    [OK] results/vif_all_heatmaps.png")
    
    # ===== PRINT SUMMARY =====
    print("\n" + "="*70)
    print("COMPLETE VIF SIMULATION RESULTS (5-13)")
    print("="*70)
    
    print("\n[FEATURE SELECTION BY VIF THRESHOLD]")
    print(combined_df[['VIF_Threshold', 'Dataset', 'Original', 'After_ChiSq', 'After_VIF', 'Final']].to_string(index=False))
    
    print("\n[CLASSIFICATION METRICS BY VIF THRESHOLD]")
    print(combined_df[['VIF_Threshold', 'Dataset', 'MCC', 'Sensitivity', 'Specificity']].to_string(index=False))
    
    # Best VIF threshold
    avg_mcc = combined_df.groupby('VIF_Threshold')['MCC'].mean().sort_values(ascending=False)
    print("\n[RANKING BY AVERAGE MCC]")
    for rank, (vif, mcc) in enumerate(avg_mcc.items(), 1):
        print(f"  {rank}. VIF={vif}: Avg MCC = {mcc:.4f}")
    
    best_vif = avg_mcc.idxmax()
    print(f"\n[RECOMMENDATION] Best VIF Threshold: {best_vif} (Avg MCC = {avg_mcc[best_vif]:.4f})")
    
    print("\n" + "="*70)
    print("[SUCCESS] EXTENDED VIF SIMULATION COMPLETE")
    print("="*70)
    print("\nOutput files:")
    print("  - results/vif_simulation_all_results.csv (complete 5-13)")
    print("  - results/vif_simulation_extended.csv (new 5,6,7,13)")
    print("  - results/vif_all_feature_selection.png")
    print("  - results/vif_all_metrics.png")
    print("  - results/vif_all_heatmaps.png")
